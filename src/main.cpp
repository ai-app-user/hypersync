...
int main(int argc, char** argv) {
    try {
        ...
        if (command == "benchmark-data-write") {
            ...
            for (std::size_t i = 1; i < args.size(); ++i) {
                if (args[i] == "--source") {
                    ...
                    if (mode == "data" || mode == "write" || mode == "data-write") {
                        ...
                        throw std::runtime_error("--mode must be data, mkdir-only, or folder-ready-discard");
                    }
                } else if (args[i] == "--non-recursive") {
                    recursive = false;
                } else if (args[i] == "--meta-reader-threads") {
            ...
            const char* mode_name = mkdir_only ? "mkdir-only" : (folder_ready_discard ? "folder-ready-discard" : "data");
