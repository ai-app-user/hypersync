...
void print_usage() {
    std::cerr
        ...
        << "  hypersync [--config <config.yaml>] benchmark-data-write --source <dir|nfs-url|synthetic-profile-url> --target <dir|nfs-url> [--mode data|mkdir-only|folder-ready-discard|folder-ready-write|folder-ready-mixed-write] [--non-recursive] [--meta-reader-threads <n>] [--metadata-async-depth <n>] [--readdirplus-page-bytes <n>] [--data-reader-threads <n>] [--data-writer-threads <n>] [--data-writer-async-window <n>] [--data-writer-file-window <n>] [--data-writer-reactors <n>] [--reactors-per-ip <n>] [--small-file-target-ips <expr>] [--large-file-target-ips <expr>] [--data-writer-stable-small-writes] [--data-writer-tcp-cork] [--data-writer-direct-reactors] [--data-writer-direct-submit] [--data-outstanding-requests <n>] [--small-file-async-window <n>] [--min-file-size-bytes <n>] [--max-file-size-bytes <n>] [--pack-small-files] [--skip-target-metadata] [--no-target-fsync] [--assume-target-directories] [--max-files-queued <n>] [--data-buffer-slots <n>] [--data-queue-depth <per-shard>] [--data-copy-mode copy|no-copy] [--verify-hash] [--max-duration-seconds <n>] [--stats-interval-seconds <n>]\n"
    ...
    try {
        ...
        if (command == "benchmark-transport" || command == "transport-speed") {
            ...
            for (std::size_t i = 1; i < args.size(); ++i) {
                ...
                } else if (args[i] == "--shared-nothing" || args[i] == "--direct-socket") {
                    shared_nothing = true;
                } else if (args[i] == "--base-port") {
                    base_port = parse_port(require_option(args, i, "--base-port"), "--base-port");
        ...
        if (command == "benchmark-data-write") {
            ...
            std::string large_file_target_ips;
            std::size_t data_outstanding_requests = 0;
            std::size_t small_file_async_window = 0;
            ...
            for (std::size_t i = 1; i < args.size(); ++i) {
                if (args[i] == "--source") {
                    ...
                    const std::string mode = require_option(args, i, "--mode");
                    if (mode == "data" || mode == "write" || mode == "data-write") {
                        mkdir_only = false;
                        folder_ready_discard = false;
                        folder_ready_write = false;
                        folder_ready_mixed_write = false;
                    } else if (mode == "mkdir-only" || mode == "mkdir" || mode == "dirs-only" ||
                               mode == "directories-only") {
                        mkdir_only = true;
                        folder_ready_discard = false;
                        folder_ready_write = false;
                        folder_ready_mixed_write = false;
                    } else if (mode == "folder-ready-discard" || mode == "folder-gated-discard" ||
                               mode == "mkdir-gated-discard") {
                        mkdir_only = false;
                        folder_ready_discard = true;
                        folder_ready_write = false;
                        folder_ready_mixed_write = false;
                    } else if (mode == "folder-ready-write" || mode == "folder-gated-write" ||
                               mode == "mkdir-gated-write") {
                        mkdir_only = false;
                        folder_ready_discard = false;
                        folder_ready_write = true;
                        folder_ready_mixed_write = false;
                    } else if (mode == "folder-ready-mixed-write" || mode == "folder-gated-mixed-write" ||
                               mode == "mixed-folder-ready-write" || mode == "mixed-write") {
                        mkdir_only = false;
                        folder_ready_discard = false;
                        folder_ready_write = false;
                        folder_ready_mixed_write = true;
                    } else {
                        throw std::runtime_error("--mode must be data, mkdir-only, folder-ready-discard, folder-ready-write, or folder-ready-mixed-write");
                    }
                } else if (args[i] == "--non-recursive") {
                    recursive = false;
                } else if (args[i] == "--meta-reader-threads") {
                    ...
                    reactors_per_ip =
                        parse_size_t_option(require_option(args, i, "--reactors-per-ip"),
                                            "--reactors-per-ip");
                } else if (args[i] == "--small-file-target-ips") {
                    small_file_target_ips = require_option(args, i, "--small-file-target-ips");
                } else if (args[i] == "--large-file-target-ips") {
                    large_file_target_ips = require_option(args, i, "--large-file-target-ips");
                } else if (args[i] == "--data-writer-stable-small-writes") {
                    stable_small_file_writes = true;
                } else if (args[i] == "--data-writer-tcp-cork") {
                    tcp_cork_small_file_writes = true;
                } else if (args[i] == "--data-writer-direct-reactors") {
                    direct_reactor_writes = true;
                } else if (args[i] == "--data-writer-direct-submit") {
                    direct_reactor_submit = true;
                } else if (args[i] == "--data-outstanding-requests") {
                    data_outstanding_requests =
                        parse_size_t_option(require_option(args, i, "--data-outstanding-requests"),
                                            "--data-outstanding-requests");
            ...
            const auto report = engine.benchmark_data_write_pipeline(source_root,
                                                                     ...
                                                                     data_writer_file_window,
                                                                     mkdir_only,
                                                                     folder_ready_discard,
                                                                     folder_ready_write,
                                                                     folder_ready_mixed_write,
                                                                     small_file_target_ips,
                                                                     large_file_target_ips);
            const char* mode_name = mkdir_only
                                        ? "mkdir-only"
                                        : (folder_ready_discard
                                               ? "folder-ready-discard"
                                               : (folder_ready_write
                                                      ? "folder-ready-write"
                                                      : (folder_ready_mixed_write ? "folder-ready-mixed-write"
                                                                                  : "data")));
            std::cout << "data_write_benchmark mode=" << mode_name
                      ...
                      << " folders_written=" << report.folders_written
                      << " folders_per_second=" << report.folders_per_second
                      << " logical_size_bytes=" << report.logical_size_bytes
                      << " bytes_read=" << report.bytes_read
