...
- 2026-05-19 PDT agnopo longer saved-profile mixed replay at commit `7bacf62`: source was `synthetic-profile://.../source-nfs-whole-100mphase-data-sampled-20260517T200342Z.profile.txt?payload=prng&files-per-batch=1024`, target was `nfs://172.27.255.2-172.27.255.17/volumes/8ed98ee4-b263-4319-be97-2093377beb65/profile-replay-mixed-60s-20260520T014505Z`, duration `60s`, pipeline was `[FolderSeeder/MetaWork-1]->(FolderQueue)->[MetaReader-SYN-96]->(FolderReadyQueue-4096)->[FolderCreation-NFS-8]->[ReadyClassifier+Spillway+Governor]->(SmallReadyFileQueue-500000)->[DataReader-SYN-768/direct-submit]->[DataWriter-NFS/reactors=64 window=64] + (MediumReadyFileQueue-500000+MediumSpillway)->[DataReader-SYN-64 governed]->(DataQueue-64x512)->[DataWriter-NFS-64] + (LargeReadyFileQueue-500000+LargeSpillway)->[DataReader-SYN-112 governed]->(DataQueue-112x512)->[DataWriter-NFS-112]`. Result: zero failures, `4,134,954` files written, `1.479 TB`, full-run average `189.47 Gbit/s`, final 60s sample `193.61 Gbit/s`, final small rate `61,767 files/s`, medium `31.50 Gbit/s`, large `131.16 Gbit/s`. Governor stayed capped at `bulk_pacing_us=2000`; small queue remained essentially empty, so the remaining gap to 200G appears to be integrated supply/orchestration rather than small-writer capacity.
- 2026-05-19 PDT copy-all acceptance from transfer1 to nopo1 used the packaged runtime from clean transfer1 staging workspace `/mnt/local-nvme/wsync-codex/copy-pipeline-dev-20260520T043418Z` at hypersync commit `16e81ad`, piper commit `45802f0`, version `hypersync 0.0.3`. Package artifact only was deployed to nopo1; no source files were copied to the remote target host. Pipeline shape: `[SourceMetaReader-FS]->(PrioritySocket)->[ReceiverMetaWriter-NFS] + [SourceDataReader-FS]->(DataSocket)->[ReceiverDataWriter-NFS]`. Source was `/mnt/local-nvme/wsync-codex/copy-source-20260520T043626Z` on transfer1. Target was `nfs://172.27.255.2-172.27.255.17/volumes/dfb990b1-bf40-4378-85f1-26f9dfd0cd2c/data/copy-accept-20260520T043642Z` on nopo1. Result: `files_total=1016`, `transferred=1016`, `skipped=0`, `failed=0`, `bytes=280723456`, `chunks_sent=322`, elapsed `9.80s`. Post-run target verification through the nopo NFS mount showed `1016` files and `280723456` bytes, and sampled source/target SHA-256 hashes matched. This proves the simple copy-all use case works end to end; sync/diff-driven selective copy is the next stage.
- 2026-05-19 PDT corrected copy-all acceptance used the real requested topology: transfer1 reads from NFS, transfers over TCP to nopo1, and nopo1 writes to NFS. Pipeline shape: `[SourceMetaReader-NFS]->(PrioritySocket)->[ReceiverMetaWriter-NFS] + [SourceDataReader-NFS]->(DataSocket)->[ReceiverDataWriter-NFS]`. Source URL: `nfs://nfs.crusoecloudcompute.com/volumes/e27faf8c-36a5-4571-8324-4c38a5dce0a5/HaWoR/video/path/0/175593000`. Target URL: `nfs://172.27.255.2-172.27.255.17/volumes/dfb990b1-bf40-4378-85f1-26f9dfd0cd2c/data/nfs2nfs-copy-20260520T053403Z`. Result: `files_total=506`, `transferred=506`, `skipped=0`, `failed=0`, `bytes=9564146`, `chunks_sent=509`, elapsed `35.84s`. Source count/bytes from the transfer1 NFS mount were `506` files and `9564146` bytes. Target sampled direct file SHA-256 hashes matched the same source files. Avoid recursive kernel-mounted target verification on this tree because `find`/`du` over the nopo kernel NFS mount can stall; rely on protocol ACKs plus bounded direct file checks or libnfs verification when available.
- 2026-05-19 PDT copy performance follow-up: committed `980e57e` to stream remote NFS large-file copy data instead of materializing whole remote files in memory, and to clamp copy-time NFS read depth to the available large slot pool to avoid an async read/slot deadlock. Local build of `build/hypersync` passed. The full `make` suite still has pre-existing CLI row-count smoke failures unrelated to this copy path (`expected 128 scan rows but saw 137`, `expected 2 scan rows but saw 4`).
- 2026-05-19 PDT legacy runtime copy perf probe after `980e57e`: using config `small_pool_slots=256`, `large_pool_slots=512`, `skip_verify=true`, `nfs_data_reader.outstanding_requests=256`, copied source `nfs://nfs.crusoecloudcompute.com/volumes/e27faf8c-36a5-4571-8324-4c38a5dce0a5/HaWoR/s1_data/v3_retarget/KYuNx13tm3to21nDAHcswQ/8wr1zuJ1EXPwmYqK6NKuLA/20260302-134737-0xt/20260302-134737-d30d7cc` to nopo target `nfs://172.27.255.2-172.27.255.17/volumes/dfb990b1-bf40-4378-85f1-26f9dfd0cd2c/data/nfs2nfs-onebig3-20260520T061250Z`. Result: `files_total=8`, `transferred=8`, `failed=0`, `bytes=13956200341`, `chunks_sent=13315`, elapsed `42.47s`, about `2.63 Gbit/s` end-to-end. Transfer1 NIC samples showed hot source NFS read around `14 Gbit/s` and TCP send up to `~10.6 Gbit/s`; nopo samples showed receive/write around `2-3 Gbit/s`. Conclusion: the legacy `copy` command is still a correctness path, not the final 200G architecture. The next implementation must replace it with the sharded job pipeline: `[MetaReader-NFS]->(FileQueue)->[DataReader-NFS-N]->(DataBufQueue-sharded-by-file)->[BufferSender-N x lanes]->TCP->[BufferReceiver-N x lanes]->(DataBufQueue-sharded-by-file)->[DataWriter-NFS/reactors]`, with directory creation/metadata as a normal job ahead of data writes.
...
- 2026-05-18 PDT: `DataWriter-NFS` already has connection sharding at worker granularity: each writer worker constructs its own `LibNfsSession` / `nfs_context`, and endpoint selection is pinned by worker index across URL ranges like `172.27.255.2-172.27.255.17`. Do not add a shared context pool for this path; it would be a regression unless the implementation explicitly preserves one context per independent lane.
- Added bounded small-file write lifecycle control: `benchmark-data-write --data-writer-file-window <n>` maps to `target_data_writer.max_concurrent_file_transactions` and `TargetWriterBackend::Options::max_concurrent_file_transactions`. Default is `64`. `LibNfsTargetWriterBackend::write_files()` now queues CREATE operations only up to the active file window, advances each file through CREATE -> WRITE -> CLOSE, and queues more files as closes complete.
...
- 2026-05-19 PDT NFS writer multi-session and corking checks:
  - `DataWriter-NFS` now supports explicit reactor fleet sizing through `--data-writer-reactors <n>` and config `target_data_writer.reactor_count`. `--reactors-per-ip <n>` / `target_data_writer.reactors_per_ip` remains available as a multiplier. Defaults still produce one reactor per target endpoint, so the 16-IP agnopo target defaults to the desired 16-reactor fleet.
  - agnopo tests with direct mode off, `window=128`, all 16 target IPs, packed synthetic small files, no fsync/metadata restore, assumed target dirs, `files-per-batch=1024`: `reactors=32` produced `16,084.6 files/s`, CPU system `49.38%`; `reactors=64` produced `12,592.7 files/s`, CPU system `44.33%`. More sockets per IP regressed throughput and did not break the plateau.
  - Installed libnfs headers expose `rpc_queue_length(struct rpc_context*)`, but no obvious public NFSv3 setter for max outstanding RPC slots/credits. NFSv4 slot structures exist in headers but do not apply to this NFSv3 writer path.
  - TCP corking is implemented only as an opt-in experiment: CLI `--data-writer-tcp-cork`, config `target_data_writer.tcp_cork_small_file_writes`. Do not enable by default on agnopo. The forced-cork 16-reactor `window=256` test produced `17,629.7 files/s`, `8.82 Gbit/s`, CPU system `54.86%`, worse than the uncorked best `18,712.7 files/s`, `9.36 Gbit/s`, CPU system `53.23%`.
  - Synthetic profile folder emission is already contract-safe interleaving: root scan seeds multiple `synthetic/batch_N` directories and each recursive lane advances by `+lane_count`. Do not change a folder batch to emit children from other folders just to smear target directory locks; that violates the folder scanner contract. Use smaller `files-per-batch` or more pre-created batch dirs for layout experiments instead.

- 2026-05-19 PDT direct-submit NFS writer path:
  - `benchmark-data-write --data-writer-direct-submit` skips the `DataQueue` and all intermediate `DataWriter` worker threads for packed-small-file NFS writes. `DataReader-*` workers submit completed buffers directly into the shared `NfsTargetWriteReactorFleet`; the reactor fleet still hashes each target path across reactors, preserving balanced distribution without the 96-worker relay.
  - This mode is distinct from `--data-writer-direct-reactors`: direct-submit keeps the shared reactor fleet and is good; direct-reactors uses lane-local reactors and was bad.
  - agnopo direct-submit sweep, all 16 target IPs, packed synthetic small files, no fsync/metadata restore, assumed dirs, `files-per-batch=1024`, `DataWriter-NFS/reactors=16`, `window=256`, stable off, cork off:
    - 96 data readers: `32,040 files/s`, `16.05 Gbit/s`, CPU avg user `11.59%`, system `7.36%`, softirq `1.74%`, idle `79.32%`.
