...
rm -rf "$bundle_dir"
mkdir -p "$bundle_dir/bin" "$bundle_dir/lib" "$bundle_dir/config" "$bundle_dir/doc"

cp "$binary" "$bundle_dir/bin/hypersync.bin"
cp "$script_dir/run-hypersync" "$bundle_dir/bin/hypersync"
chmod 0755 "$bundle_dir/bin/hypersync" "$bundle_dir/bin/hypersync.bin"

if [[ -f "$hypersync_repo/config/default.yaml" ]]; then
  cp "$hypersync_repo/config/default.yaml" "$bundle_dir/config/default.yaml"
fi
...
copy_library() {
  ...
  cp -L "$real" "$bundle_dir/lib/$real_base"

  if [[ -n "$soname" && "$soname" != "$real_base" ]]; then
    ln -sf "$real_base" "$bundle_dir/lib/$soname"
  fi

  local path_base
  path_base="$(basename "$path")"
  if [[ "$path_base" != "$real_base" && "$path_base" != "$soname" ]]; then
    ln -sf "$real_base" "$bundle_dir/lib/$path_base"
  fi
}
...
cat > "$bundle_dir/doc/README.txt" <<'README'
Hypersync Linux deploy bundle

Run:
  ./bin/hypersync --version

The bin/hypersync wrapper sets LD_LIBRARY_PATH to this bundle's lib directory
before executing bin/hypersync.bin.
...

{
  ...
  echo
  echo "[ldd]"
  LD_LIBRARY_PATH="$bundle_dir/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" ldd "$bundle_dir/bin/hypersync.bin" || true
...
LD_LIBRARY_PATH="$bundle_dir/lib${LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}" "$bundle_dir/bin/hypersync" --version
